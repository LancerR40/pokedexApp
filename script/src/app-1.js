...
const pokemonCard = pokemon => {
    ...
    card(accessChange, pokemonCard, IDPokemon, pokemonName, pokemonType, pokemonImg);
};
...
const loadNum2 = async JSONFile => {
    ...
    if(JSONFile.countChange >= 1) {
        ...
        if(loadCount == 1) {
            ...
            if(countChangeArray[lastedArrayIndex] == countArray[penultimateArrayIndex]) {
                internalLoadCount++;
...
const searchPokemon = async() => {
    ...
    if(takeID != '') {
        ...
        const pokemonName = pokemon.name[0].toUpperCase() + pokemon.name.slice(1);
        const pokemonType = pokemon.types.map(type => type.type.name);
        const pokemonImg = `https://pokeres.bastionbot.org/images/pokemon/${takeID}.png`;
...
const card = (accessChange, pokemonCard, IDPokemon, pokemonName, pokemonType, pokemonImg) => {
    let typeArray1 = [];
    let typeArray2 = [];
    ...
    let thisColor = `<div style = 'display: flex; justify-content:center;'>${typeLength == 1 ? colorBox1 : colorBox2}</div>`;
    ...
    const innerHTML = `
        ...
        <div class = 'pokemon-data'>
            <span class = 'pokemon-id'> #${IDPokemon.toString().padStart('3', 0)} </span>
    ...
    if(accessChange == false) {
        ...
        pokemonCard.onclick = function() {
            const url = `./views/pokemon.html?id=${IDPokemon}`;
            location.href = url;
        };
