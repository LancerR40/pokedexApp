...
const dataManagement = (packNum1, packNum2, packNum3) => {
    ...
    const pokemonImg = `https://pokeres.bastionbot.org/images/pokemon/${packNum1.id}.png`;

    // OTHER DATA
    const pokemonGroup = packNum2.egg_groups.map(group => group.name);
    const pokemonGender = packNum2.gender_rate;
    const pokemonAbility = packNum1.abilities.map(ability => ability.ability.name);

    // POKEMON HABITAT
    const pokemonHabitat = packNum2.habitat;

    const habitatAnalysis = pokemonHabitat != null ? pokemonHabitat.name : 'has no habitat';
    ...
    const thisColor = `<div style = 'display: flex; justify-content:center;'>${typeLength == 1 ? colorBox1 : colorBox2}</div>`;
...
const pokemonName = pokemon => {
    ...
    const innerChange = `
        <div class = 'icon-num1'> <i class="fas fa-chevron-left" onclick = 'back(${pokemon[0]})'></i> </div>

        <div class = 'id-box'>
            <span> N.°${pokemon[0].toString().padStart(3, '0')} </span>
        </div>

        <div class = 'icon-num2'> <i class="fas fa-chevron-right" onclick = 'next(${pokemon[0]})'></i> </div>
    `;
...
const qualitiesRequest = async JSONFile => {
    ...
    for(let i = 0; i < url.length; i++) {
        ...
        // GET QUALITIES
        const dataNum1 = res.data.damage_relations.double_damage_to.map(element => element.name);

        const dataNum2 = res.data.damage_relations.double_damage_from.map(element => element.name);
...
const pokemonEvolution = async(pass, pack) => {
    ...
    if(pass == 3) {
        ...
        for(let i = 0; i < IDArray.length; i++) {
            const pokemonImg = `https://pokeres.bastionbot.org/images/pokemon/${IDArray[i]}.png`;
        ...
        const innerHTML = `
            ...
            <div class = 'evolution-box'>
                <div id = 'evolution-data-box-1' class = 'evolution-data-box' onclick = 'newPokemon(${IDArray[0]})'>
                    <div class = 'img-card'>
                        <img src = '${imgArray[0]}'>
                    </div>
                    ...
                    <div class = 'data-box'>
                        ...
                        <span class = 'pokemon-id'> N°${IDArray[0].toString().padStart(3, '0')} </span>
                    </div>
                ...
                <div id = 'evolution-data-box-2' class = 'evolution-data-box' onclick = 'newPokemon(${IDArray[1]})'>
                    <div class = 'img-card'>
                        <img src = '${imgArray[1]}'>
                    </div>
                    ...
                    <div class = 'data-box'>
                        ...
                        <span class = 'pokemon-id'> N°${IDArray[1].toString().padStart(3, '0')} </span>
                    </div>
                ...
                <div id = 'evolution-data-box-3' class = 'evolution-data-box' onclick = 'newPokemon(${IDArray[2]})'>
                    <div class = 'img-card'>
                        <img src = '${imgArray[2]}'>
                    </div>
                    ...
                    <div class = 'data-box'>
                        ...
                        <span class = 'pokemon-id'> N°${IDArray[2].toString().padStart(3, '0')} </span>
                    </div>
...
const newPokemon = IDPokemon => {
    location.href = `./pokemon.html?id=${IDPokemon}`;
};
